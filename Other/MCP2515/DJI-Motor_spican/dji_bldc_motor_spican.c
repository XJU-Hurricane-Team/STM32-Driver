...
#include "dji_bldc_motor_spican.h"


/**
 * @brief SPICAN 收到消息中断回调
 *
 * @param node_obj 节点数据
 * @param spican_rx_header CAN 消息头
...
static void spican_callback(void *node_obj, spican_rx_header_t *can_rx_header,
    ...
    motor_point->angle = (uint16_t)((spican_msg[0] << 8) | spican_msg[1]);
    ...
    switch (motor_point->motor_model) {
        ...
        case DJI_M3508: {
            ...
            motor_point->given_current =
                (int16_t)((float)(spican_msg[4] << 8 | spican_msg[5]) / -5.0f);
        } break;
...
uint8_t dji_motor_init_mcp2515(dji_motor_mcp_handle_t *motor, dji_motor_model_mcp_t motor_model,
    ...
    if (spican_list_add_new_node(spican_select, (void *)motor, can_id, 0x7FF,
