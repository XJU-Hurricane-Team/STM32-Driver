# STM32 驱动



存放一些经过验证的驱动代码。主要是 STM32 外设和电机的驱动程序。

全部支持战队使用的[工程模板](https://github.com/Deadline039/STM32-Template)，添加到`BSP`层即可使用。

代码注释采用 doxygen 格式。后面会考虑将代码中的 doxygen 转换成 markdown，以方便查阅。

某些代码即使验证了也可能存在 bug，而且 README 中的 demo 不保证 100% 可用。欢迎 Issue 或 PR。



### `Motor`文件夹

| 模块          | 说明                                              | 是否验证 |
| ------------- | ------------------------------------------------- | -------- |
| can_list      | CAN 回调链表，所有 CAN 通信的电机都需要依赖此模块 | 是       |
| AK-Motor      | CubeMars AK 系列电机                              | 是       |
| DJI-Motor     | 大疆 M3508/2006/GM6020                            | 是       |
| VESC          | VESC 电调驱动                                     | 是       |
| step_motor    | 定时器PWM驱动步进电机驱动代码                     | 是       |
| MLDS          | 铭朗科技 伺服电机                                 | 否       |
| unitree_motor | 宇树GO-M8010-6电机，485通信                       | 是       |
| atk_smd       | 正点原子步进电机驱动，485通信                     | 是       |



### `Display`文件夹

| 模块      | 说明                                                        | 是否验证 |
| --------- | ----------------------------------------------------------- | -------- |
| OLED      | 支持0.91，0.96，1.3寸iic接口的oled基本驱动                  | 是       |
| OLED_RTOS | 支持0.91，0.96，1.3寸spi或iic接口的oled驱动，使用freertos。 | 是       |
| st77xx    | 1.8寸spi接口LCD屏幕驱动,支持st7735与st7789.                 | 是       |



### `Sensor`文件夹

| 模块            | 说明                    | 是否验证  |
| --------------- | ----------------------- | --------- |
| Myantenna-L1    | 激光测距模块代码        | 见 readme |
| N300            | 轮趣科技 12轴高精度IMU  | 是        |
| DT35            | 4mA~20mA距离传感器      | 是        |
| stp23           | stp23距离传感器         | 是        |
| as5047          | 16位spi接口磁编码器     | 否        |
| action_position | 码盘定位接收解析代码    | 是        |
| bmp100          | 气压计                  | 是        |
| VL53L1          | 基于ToF的激光测距传感器 | 是        |



### `Memorizer`文件夹

| 模块    | 说明              | 是否验证 |
| ------- | ----------------- | -------- |
| w25qxx  | NOR Flash芯片驱动 | 否       |
| at24cxx | eeprom芯片驱动    | 否       |
|         |                   |          |



### Resolve文件夹

| 模块           | 说明           | 是否验证 |
| -------------- | -------------- | -------- |
| omni_wheels    | 全向轮解算代码 | 否       |
| steering_wheel | 舵轮解算代码   | 否       |
|                |                |          |



### Other文件夹

| 模块    | 说明     | 是否验证 |
| ------- | -------- | -------- |
| iic     | 软件iic  | 是       |
| MCP2515 | SPI转CAN | 是       |
|         |          |          |



### `Utils`文件夹

| 模块              | 说明                           | 是否验证 |
| ----------------- | ------------------------------ | -------- |
| a_star            | A*路径规划算法，仅四方向移动。 | 是       |
| theta_star        | θ*路径规划算法，八方向移动。   | 是       |
| bcc               | BCC校验相关工具                | 否       |
| buffer_append     | 缓冲区追加工具                 | 是       |
| crc               | CRC校验工具                    | 是       |
| kalman            | 卡尔曼滤波                     | 是       |
| linear_regression | 线性回归                       | 是       |
| low_pass_filter   | 低通滤波                       | 是       |
| my_math           | 常用数学工具                   | 是       |
| pid               | PID控制器                      | 是       |
| trajectory_plan   | 轨迹规划                       | 是       |
| ring_fifo         | 循环队列                       | 是       |